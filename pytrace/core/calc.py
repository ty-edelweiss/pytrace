...
class CalcTrace(object):
    ...
    def calc(self, times: List[datetime.datetime], ref_time: datetime.datetime) -> List[Tuple[datetime.datetime, datetime.datetime]]:
        traces = []
        ...
        if ref_time is None:
            ...
            traces.extend([self.window(ref_time, n, True) for n in range(1, window_numbers + 1)])
            traces.append(((ref_time, ref_time), 0))
            traces.extend([self.window(ref_time, n) for n in range(1, window_numbers + 1)])
        return traces
